...
The results indicate that in terms of prediction accuracy a model with least variables per split (2) and a minimal node size of 1 is sufficient enough.
...
plot_ly(rfProfile$resample, y = ~Accuracy, color = ~as.factor(Variables), colors= viridis_pal(option = "D")(26), type = "box")
...
kable(rfProfile$results[whichTwoPct,], row.names = F) %>% kable_styling(full_width = F)
...
The *predictor variables* are listed in \@ref(tab:rfe_tab_predictors_simp):
