...
<!-- # load bi data -->
<!-- bi <- read.csv(here("data/reference/bi/bi_ready.csv"), stringsAsFactors=TRUE) -->

<!-- # join species groups -->
<!-- spec_code <- read.csv2(here("data/reference/ba_code_ni_bdat.csv"), stringsAsFactors=TRUE) -->
<!-- bi <- bi %>% -->
<!--   left_join(spec_code %>% select(id_ni, gruppe), by = c("species" = "id_ni")) -->


<!-- # calculate species shares per plot (by basal area, crown projection) -->
<!-- bi_plots <- bi %>% -->
<!--   mutate(ba = pi*(0.5*dbh)^2, -->
<!--          crown = pi*(0.5*crownwidth)^2) %>% -->
<!--   group_by(id) %>% -->
<!--   mutate(ba_plot = sum(ba), -->
<!--          crown_plot = sum(crown)) %>% -->
<!--   ungroup() %>% -->
<!--   group_by(id, datph2_rw, datph2_hw, gruppe) %>% -->
<!--   summarise(ba = sum(ba)/unique(ba_plot), -->
<!--             crown = sum(crown)/unique(crown_plot),) %>% -->
<!--   ungroup() -->

<!-- # species per plot with max basal area -->
<!-- bi_spec <- bi_plots %>% -->
<!--   group_by(id, datph2_rw, datph2_hw) %>% -->
<!--   slice_max(ba) %>% -->
<!--   ungroup() -->
...
<!-- # make spatial -->
<!-- bi_spec.sp<- SpatialPointsDataFrame(coords = bi_spec[,c("datph2_rw", "datph2_hw")], data = bi_spec, proj4string = CRS("+init=epsg:31467")) -->

<!-- # transform -->
<!-- bi_spec.sp <- spTransform(bi_spec.sp, img.class@crs) -->

<!-- bi_spec_sol.sp <- crop(bi_spec.sp, ext.sol) -->
<!-- bi_spec_har.sp <- crop(bi_spec.sp, ext.har) -->
<!-- bi_spec_hei.sp <- crop(bi_spec.sp, ext.hei) -->


<!-- # extract classification results at bi plots -->
<!-- ref.class.bi <- raster::extract(img.class, bi_spec.sp, df=TRUE) -->
<!-- ref.class.bi$plot_id <- bi_spec.sp$id -->
<!-- # rename classification -->
<!-- colnames(ref.class.bi)[2] <- "classification" -->
<!-- # merge data -->
<!-- ref.class.bi <- merge(bi_spec, ref.class.bi ,by.x="id", by.y="plot_id", all = TRUE) -->
<!-- # rename reference -->
<!-- ref.class.bi <- ref.class.bi %>% rename(reference = gruppe) -->
<!-- # revalue classification data -->
<!-- ref.class.bi$classification <- as.factor(ref.class.bi$classification) -->

<!-- ref.class.bi$classification <- plyr::revalue(ref.class.bi$classification, c("6"="EI", -->
<!--                     "1"="BU", -->
<!--                     "3"="FI", -->
<!--                     "2"="DGL", -->
<!--                     "4"="KI", -->
...
<!-- cm<- confusionMatrix(data = ref.class.bi$classification, reference = ref.class.bi$reference) -->
<!-- cm.df <- as.data.frame.matrix(cm$table) -->
...
BI-plots within the study areas were used as validation data. Forest stand type assessed in the latest inventory up to 2018 was aggregated by species group on plot level. Plots dominated by other species groups (ALH, ALN) were not considered.
Class predictions for these plots were extracted from the model prediction raster and compared against the inventory data.
...
bi_sol.sp <- crop(bi.sp, ext.sol)
bi_har.sp <- crop(bi.sp, ext.har)
bi_hei.sp <- crop(bi.sp, ext.hei)
